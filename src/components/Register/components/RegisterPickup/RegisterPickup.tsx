...
const RegisterPickupInner: React.FC = () => {
  ...
  const handleStepClick = (clickedStep: number) => {
    if (clickedStep <= maxStepReached) { setStep(clickedStep); window.scrollTo({ top: 0, behavior: 'smooth' }); }
  };

  const handleGameSelect = (game: PickupGame) => setSelectedGame(game);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const total = (selectedGame?.pricePerPlayer || 0) * formData.spots;
      const { clientSecret } = await api.createPaymentIntent(
        total * 100,
        `Pickup Game - ${selectedGame?.format} - ${selectedGame?.date}`
      );
      ...
      await api.createBooking({
        ...
        metadata: { game_id: selectedGame?.id, format: selectedGame?.format, spots: formData.spots },
      });
      ...
      await sendEmail({
        ...
        booking: {
          ...
          service: 'pickup',
          bookingDate: selectedGame?.date || '',
          totalAmount: total,
          metadata: { format: selectedGame?.format, time: selectedGame?.time },
        },
      });
      ...
      setTimeout(() => {
        ...
        setMaxStepReached(1);
        setFormData({ spots: 1, name: '', email: '', phone: '' });
        cardNumberEl.clear();
      }, 3000);
    } catch (error: any) {
        ...
        <div className={styles.contentCard}>
          ...
          {step === 1 && (
            <div className={styles.step}>
              ...
              <div className={styles.weekNavigation}>
                <button className={styles.weekNavBtn} onClick={() => setSelectedWeek(selectedWeek - 1)} disabled={selectedWeek === 0}>
                  <ChevronLeft size={20} />
                </button>
                <span className={styles.weekRange}>{weekStart} - {weekEnd}</span>
                <button className={styles.weekNavBtn} onClick={() => setSelectedWeek(selectedWeek + 1)} disabled={selectedWeek >= 3}>
              ...
              <div className={styles.scheduleVertical}>
                {weekDates.map((date, index) => {
                  ...
                  return (
                    <div key={index} className={`${styles.dayRow} ${isToday ? styles.today : ''} ${index % 2 === 1 ? styles.alt : ''}`}>
                          ...
                          games.map(game => (
                            ...
                            >
                              <div className={styles.gameCardTop}>
                                <div className={styles.gameTime}><Clock size={15} />{game.time}</div>
                              ...
                              <div className={styles.gameCardMiddle}>
                                <span className={styles.formatBadge}><Trophy size={12} />{game.format}</span>
                                <span className={styles.gameSkill}>{translateSkillLevel(game.skillLevel)}</span>
                              </div>
                              <div className={styles.gameCardBottom}>
                                <div className={styles.gameInfo}><MapPin size={14} />{game.location}</div>
            ...
            <div className={styles.step}>
              ...
              <div className={styles.section}>
                ...
                <div className={styles.gameDetails}>
                  <div className={styles.detailRow}><Calendar size={20} /><span>{new Date(selectedGame.date).toLocaleDateString('en-US', { weekday: 'long', month: 'long', day: 'numeric' })}</span></div>
                  <div className={styles.detailRow}><Clock size={20} /><span>{selectedGame.time}</span></div>
                  <div className={styles.detailRow}><MapPin size={20} /><span>{selectedGame.location}</span></div>
                  <div className={styles.detailRow}><Trophy size={20} /><span>{selectedGame.format} - {translateSkillLevel(selectedGame.skillLevel)}</span></div>
                  <div className={styles.detailRow}><Users size={20} /><span>{selectedGame.spotsAvailable} {t('register.pickup.details.spotsAvailable')}</span></div>
                </div>
              </div>
              <div className={styles.section}>
                <h3 className={styles.sectionTitle}>
                ...
                <div className={styles.formGroup}>
                  ...
                  <label className={`${styles.floatingLabel} ${styles.active}`}>{t('register.pickup.details.numberOfSpots')}</label>
                </div>
                <div className={styles.priceHighlight}>${selectedGame.pricePerPlayer * formData.spots}</div>
              </div>
            </div>
          )}
            ...
            <div className={styles.step}>
              ...
              <div className={styles.bookingDetailsWrapper}>
                <div className={styles.bookingDetails}>
                  ...
                  <div className={styles.detailRow}><MapPin size={20} /><span>{selectedGame.location}</span></div>
                  <div className={styles.detailRow}><Trophy size={20} /><span>{selectedGame.format} - {translateSkillLevel(selectedGame.skillLevel)}</span></div>
                  <div className={styles.detailRow}><User size={20} /><span>{formData.name}</span></div>
                  <div className={styles.detailRow}><Mail size={20} /><span>{formData.email}</span></div>
                </div>
          ...
          <div className={styles.actions}>
            ...
            {step < 5 ? (
              ...
              <button className={`${styles.button} ${styles.buttonPrimary}`} onClick={handleSubmit} disabled={isSubmitting}>
