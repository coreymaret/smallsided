...
const AdminLeagues = () => {
  ...
  const fetchRegistrations = async () => {
    try {
      let query = supabase
        ...
        .select('*, leagues(*)');
      if (range.from) query = (query as any).gte('registration_date', range.from);
      if (range.to)   query = (query as any).lte('registration_date', range.to);
  ...
  // League registrations don't have booking_status — we track payment_status only
  // The drawer is used for contact info, notes, and read-only detail view
  const handleRowClick = (r: LeagueRegistration) => {
    setSelectedBooking({
      ...
      booking_date: r.registration_date ?? r.created_at.split('T')[0],
      start_time: null,
      end_time: null,
      field_id: null,
      participants: null,
      ...
      booking_status: 'confirmed', // leagues are confirmed on registration
  ...
  // Status changes on league registrations update payment_status, not booking_status
